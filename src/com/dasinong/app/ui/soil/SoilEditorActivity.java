...
public class SoilEditorActivity extends SoilBaseActivity implements View.OnClickListener {
	...
	private void postSoilInformation(SoilPostEntity.Param param) {
		...
		if (isEditor){
		  ...
		  url = URL_INSERT_SOIL;
		  code = REQUEST_CODE_SOIL_INSERT;
		}
		DEBUG("isUpdate..." + isEditor);
		DEBUG("isUpdate..." + url);
		VolleyManager.getInstance().addPostRequest(code, url, param, BaseResponse.class, this);
	}

	private void checkStatus() {
		if ((originday == day && originmonth == month && originyear == year) && mListDataEntity!=null) {
				isEditor = true;
		}else{
			isEditor = false;
		}
...
// &&
                 ...
                 if (mListDataEntity!=null){
                	 param.reportId = mListDataEntity.soilTestReportId;
                 }

                param.fieldId = "" + SharedPreferencesHelper.getLong(this, Field.FIELDID, 10);
                param.userId = SharedPreferencesHelper.getString(this, Field.USER_ID, "10");
                
                
                param.type = (!TextUtils.isEmpty(mSoilType.getText())) ? mSoilType.getText().toString() : "";
                param.color = (!TextUtils.isEmpty(mSoilColor.getText())) ? mSoilColor.getText().toString() : "";
                param.fertility = (!TextUtils.isEmpty(mSoilFertility.getText())) ? mSoilFertility.getText().toString() : "";
                param.phValue = (!TextUtils.isEmpty(mSoilPH.getText())) ? mSoilPH.getText().toString().trim() : "0";
                param.organic = (!TextUtils.isEmpty(mSoilOrganics.getText())) ? mSoilOrganics.getText().toString().trim() : "0";
                param.p = (!TextUtils.isEmpty(mSoilP.getText())) ? mSoilP.getText().toString().trim() : "0";
                param.an = (!TextUtils.isEmpty(mSoilAN.getText())) ? mSoilAN.getText().toString().trim() : "0";
                param.qn = (!TextUtils.isEmpty(mSoilQN.getText())) ? mSoilQN.getText().toString().trim() : "0";
                param.qK = (!TextUtils.isEmpty(mSoilQK.getText())) ? mSoilQK.getText().toString().trim() : "0";
                param.sK = (!TextUtils.isEmpty(mSoilSK.getText())) ? mSoilSK.getText().toString().trim() : "0";
                param.mo = (!TextUtils.isEmpty(mSoilMO.getText())) ? mSoilMO.getText().toString().trim() : "0";
                param.fe = (!TextUtils.isEmpty(mSoilFE.getText())) ? mSoilFE.getText().toString().trim() : "0";
                param.mn = (!TextUtils.isEmpty(mSoilMN.getText())) ? mSoilMN.getText().toString().trim() : "0";
                param.cu = (!TextUtils.isEmpty(mSoilCU.getText())) ? mSoilCU.getText().toString().trim() : "0";
                param.zn = (!TextUtils.isEmpty(mSoilZN.getText())) ? mSoilZN.getText().toString().trim() : "0";
                param.b = (!TextUtils.isEmpty(mSoilB.getText())) ? mSoilB.getText().toString().trim() : "0";
                param.ca = (!TextUtils.isEmpty(mSoilCA.getText())) ? mSoilCA.getText().toString().trim() : "0";
                param.s = (!TextUtils.isEmpty(mSoilS.getText())) ? mSoilS.getText().toString().trim() : "0";
                param.si = (!TextUtils.isEmpty(mSoilSI.getText())) ? mSoilSI.getText().toString().trim() : "0";
                param.mg = (!TextUtils.isEmpty(mSoilMG.getText())) ? mSoilMG.getText().toString().trim() : "0";
